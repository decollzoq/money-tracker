...
function TransactionInputModal({onClose, onSave, editTarget}){
    ...
    useEffect(()=>{
        if(editTarget){
            ...
            setInputDate(new Date(editTarget.date).toISOString().substring(0, 10));
        }
    },[editTarget])
        ...
        <div className="modal-background">
            <div className="modal-container">
                ...
                <div className="type-toggle-group">
                    <button 
                    className={isSpendingType ? "toggle-unselected" : "toggle-selected"} 
                    onClick={()=>setIsSpendingType(false)}>수입</button>

                    <button
                    className={isSpendingType ? "toggle-selected" : "toggle-unselected"} 
                    onClick={()=>setIsSpendingType(true)}>지출</button>
                </div>
                ...
                <div className="input-group">
                    <input 
                        ...
                        value={inputAmount}/>
                    <span className="wonStyle">원<span className="starStyle"> *</span></span>
                </div>
                ...
                <div className="button-group">
                    <button className="button-close-modal" onClick={onClose}>취소</button>
                    <button className="button-add-transaction" onClick={handleSave}>확인</button>
                </div>
